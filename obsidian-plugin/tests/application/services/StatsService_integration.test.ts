...
describe('StatsService Integration', () => {
    ...
    test('fetchAnkiCardStats calls getFSRSStats and merges difficulty', async () => {
        ...
        (requestUrl as jest.Mock).mockResolvedValueOnce({
            json: {
                result: [
                    { cardId: 1, note: 101, lapses: 0, factor: 0, interval: 10, due: 0, reps: 5, deckName: 'Deck A', difficulty: 0 }, // difficulty 0 ignored if FSRS fetched
                    { cardId: 2, note: 102, lapses: 0, factor: 0, interval: 10, due: 0, reps: 5, deckName: 'Deck A', difficulty: 0 }
                ]
            }
        });
    ...
    test('fetchAnkiCardStats handles missing FSRS data gracefully', async () => {
        ...
        (requestUrl as jest.Mock).mockResolvedValueOnce({
            json: {
                result: [
                    { cardId: 1, note: 101, lapses: 0, factor: 0, interval: 10, due: 0, reps: 5, deckName: 'Deck A', difficulty: 0.5 } // Info fallback
                ]
            }
        });
