...
describe('O2APlugin CLI Interaction', () => {
	...
	test('runSync logs stdout output', async () => {
		...
		await syncPromise;
		expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('STDOUT: Processing note 1'));
    ...
    test('checkVaultIntegrity issues flow', async () => {
        ...
        await plugin.checkVaultIntegrity();
        expect(Notice).toHaveBeenCalledWith(expect.stringContaining('Found 1 files with Invalid Properties'));
    });
    ...
    test('runCheck handles success', async () => {
		const checkPromise = plugin.runCheck('/path/to/file.md');
		mockChild.stdout.emit('data', JSON.stringify({ ok: true, stats: { deck: 'Default', cards_found: 2 } }));
		mockChild.emit('close', 0);
		await checkPromise;
    ...
    test('runFix success with .py script', async () => {
        ...
        expect(spawn).toHaveBeenCalledWith(
            ...
            expect.objectContaining({ env: expect.objectContaining({ PYTHONPATH: expect.any(String) }) })
        );
    });
