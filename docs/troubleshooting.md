...
This guide covers common issues encountered while using ObsiAnki (o2a) and how to resolve them.
...
-   **Allowed Origins**: In Anki, go to `Tools -> Add-ons -> AnkiConnect -> Config` and ensure your origin is allowed. For o2a, adding `*` to `webBindAddress` (if needed) or ensuring `localhost` is in `webCorsOriginList` helps.

## 2. WSL (Windows Subsystem for Linux)

WSL users often face networking hurdles because WSL 2 uses a virtualized network.

-   **The `curl.exe` Bridge**: `o2a` attempts to use `curl.exe` (the Windows host binary) to communicate with Anki running on Windows. Ensure `curl.exe` is in your Windows PATH and accessible from WSL.
-   **Firewall**: Ensure Windows Firewall isn't blocking incoming connections to port 8765.

## 3. Python Environment (`uv`)

-   **`Command not found: o2a`**: Ensure you are running commands with `uv run o2a` or that your virtual environment is activated.
...
-   **Cache Mismatch**: If you suspect the cache is out of sync, you can force a re-scan by deleting the cache file:
    ```bash
    rm ~/.config/o2a/cache.db
    ```
    *(Path may vary based on OS; use `o2a config show` to find your config directory).*
